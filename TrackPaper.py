...
# Doc file video
cap = cv2.VideoCapture("testDetectPaper.mp4")
amountOfFrame = int(cap.get(cv2.CAP_PROP_FRAME_COUNT));  # Dem khung hinh
print(amountOfFrame);
...
while True:
    ...
    # amountOfFrame = int(cap.get(cv2.CAP_PROP_FRAME_COUNT));  # Dem khung hinh (khoảng 30 fps)
    # print(amountOfFrame);
    ...
    if key == ord('q'):
        break
    if key == ord('s'):  # Select lai ROI moi
        select_box = cv2.selectROI(frame, showCrosshair=True)
